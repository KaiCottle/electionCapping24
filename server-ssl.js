...
app.use(
  session({
    secret: 'c2d6e1f1b4a6e0f3c5d8e7f1a2b3c4d5e6f7081920a1b2c3d4e5f6a7b8c9d0e1',
    resave: false,
    saveUninitialized: true,
    cookie: { secure: true }, // Set secure to true since we're using HTTPS
  })
);
  ...
  new SamlStrategy(
    {
      ...
      privateCert: fs.readFileSync('/var/www/html/backend/sp-key.pem', 'utf-8'), // Use the private key for privateCert
      identifierFormat: null,
      decryptionPvk: fs.readFileSync('/var/www/html/backend/sp-key.pem', 'utf-8'), // Use the private key for decryptionPvk
...
app.post('/admin-login', async (req, res) => {
  ...
  try {
    ...
    if (hashedInputPassword !== admin.thepassword) {
      // Use correct case for thepassword
      return res.status(400).json({ message: 'Invalid username or password' });
    }
...
// Create HTTPS server on a non-privileged port (e.g., 8443)
